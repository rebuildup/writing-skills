...
この文書は、writing-skills の開発組織における最上位 contract です。
...
writing-skills の開発組織の目的は current procedure への服従ではありません。
...
本 repository は writing-oriented Agent Skills の canonical source である。project-init 由来の運用Skillは repository 開発を支援する project-local dependency であり、`skills/` 配下の writing-skills 自身の published source ownership を置換しない。

## Upstream source

`rebuildup/project-init@release-0-3-0` (`57fb4a2e5abe6f52e4fd9cb2cb88234496e47d4b`) から governance baseline を reconcile した。Agent Skills はこの revision に freeze せず current upstream を追跡する。
