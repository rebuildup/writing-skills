...
- repository product artifacts under `skills/` are authored/distributed by writing-skills and must not be overwritten by project-init updates.
- project-init Skills used for repository development live in CLI-managed project-local agent directories, separate from distributable writing Skills.
- reader-oriented writing discipline remains the product-specific canonical behavior.
...
- project-local only; `skills-lock.json` is CLI-generated and committed
- project-specific behavior stays outside upstream-managed Skill files
